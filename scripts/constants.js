// Colours
const COLOUR_BG = 'rgba(101,104,220)';
const COLOURS_PARTS = ['rgba(306,333,50,0.7)','rgba(206,233,50,0.8)', 'rgba(196,183,50,0.6)', 'rgba(356,213,50,0.9)'];

// Default physical values for Stoke's Law (NOTE: arbitrary)
const DEFAULT_RADIUS = 15;
const DEFAULT_RHO_P = 1.05;
const DEFAULT_RHO_S = 1.00;
const DEFAULT_GFORCE = 9.00; //BUG: doesn't this need to be acceleration constant multiplied by 1 (default)?
const DEFAULT_VISCOSITY = 10;

// Empirical constant used in Stoke's Law
const EMPIRICAL_CONSTANT = 18;
